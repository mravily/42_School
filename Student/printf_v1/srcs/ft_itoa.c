...
/*   Updated: 2019/12/12 16:51:41 by mravily          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			*ft_itoa(t_data *data, t_tool_box *t_box)
{
	...
	//printf("4rd_nbr = %d\n", nbr);
	i = 0;
	if ((t_box->pad_space == TRUE && t_box->pad_dir == LEFT) ||
	(t_box->arg_neg == TRUE && t_box->pad_zero == FALSE && t_box->pad_dir == LEFT && t_box->precision == FALSE))
	{
		//PRINTPOS;
